...
def draw_picked_points_on_image(image, points, cam_params, fov_deg, radius=8, color=(0, 255, 0), thickness=2):
    ...
    h, w = image.shape[:2]
    for pt in points:
        proj = world_point_to_pixel_and_depth(torch.tensor(pt), cam_params.cpu(), fov_deg, w, h)
        px, py = int(round(proj[0].item())), int(round(proj[1].item()))
...
class Pick3DWidget(Widget):
    ...
    def __call__(self, show=True):
        ...
        viz.store.picked_points = [p.tolist() for p in self.points]  # for orbital validation widget
